...
bool intersect(Point a1, Point a2, Point b1, Point b2)
{
  ...
  if (((d1 > 0 && d2 < 0) || (d1 < 0 && d2 > 0)) &&
    ...
    return (fabs(d1) < EPS && on_segment(b1, b2, a1)) ||
      (fabs(d2) < EPS && on_segment(b1, b2, a2)) ||
      (fabs(d3) < EPS && on_segment(a1, a2, b1)) ||
      (fabs(d4) < EPS && on_segment(a1, a2, b2));
  }
}
...
bool see(int p)
{
  ...
  for (int i = 0; i < P; i++) {
    ...
    if (dot(person[p][0], treasure, person[p][0], person[i][0]) > EPS &&
	fabs(direction(person[p][0], person[i][0], treasure)) < EPS &&
